...
int SLAVE_ADDRESS_LCD;

I2C_HandleTypeDef lcd_i2c_handl;

// Function to initialize the LCD
void lcd_init(I2C_HandleTypeDef *hi2c, int adr)
{
	lcd_i2c_handl=*hi2c;
	SLAVE_ADDRESS_LCD=adr;
...
HAL_StatusTypeDef lcd_send_cmd(uint8_t cmd)
{
    ...
    if (HAL_I2C_Master_Transmit(&lcd_i2c_handl, SLAVE_ADDRESS_LCD << 1, (uint8_t *)data_t, 4, HAL_MAX_DELAY) != HAL_OK)
    {
        // Handle error
        return HAL_ERROR;
    }
...
uint8_t scan_i2c_address(void)
{
    ...
    for (i = 1; i < 128; i++)
    {
        result = HAL_I2C_IsDeviceReady(&lcd_i2c_handl, (uint16_t)(i << 1), 1, 10);
        if (result == HAL_OK)
        {
            //snprintf(buf,sizeof(buf),"I2C device found at address 0x%02X\n", i);
            return i;
        }
    }
